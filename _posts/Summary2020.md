...
小数精度:二进制指数(粒度2=0.00000011920928955078125)不能完全表示十进制小数。且可知十进制的小数点后6位为可靠数字位,后面则需要近似。

整数精度:float型只有24位有效数字位,只能准确表示24位有效二进制位的整数(int型有31位)。

转换成十进制后的可靠数字位数:单精度(1+6),双精度(1+15)
